...
export const paramNameMismatch = createRule<Options, MessageIds>({
  ...
  meta: {
    type: 'problem',
    docs: {
      description: 'Ensure param name in c.req.param() matches the route definition',
    },
    schema: [],
    messages: {
      paramMismatch:
                 ...
                 if (!definedParams.includes(usedParam)) {
                   context.report({
                     ...
                     data: {
                       paramName: usedParam,
                       routePath,
                       expectedParams: definedParams.length > 0 ? definedParams.join(', ') : '(none)',
                     },
                   });
                 }
              }
              ...
              if (arg.params.length > 0) {
                 ...
                 if (contextParam.type === 'Identifier') {
                   ...
                   if (arg.body.type === 'BlockStatement') {
                      validateHandler(arg.body, contextName, definedParams, routePath);
                   } else {
                      validateHandler(arg.body, contextName, definedParams, routePath);
                   }
                 }
